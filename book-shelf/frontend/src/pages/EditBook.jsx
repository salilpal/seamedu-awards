...
const EditBook = () => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        ...
        setBook({ title, author, publishYear: publishYear.toString() }); // Convert publishYear to string for input
        setLoading(false);
      })
      .catch((error) => {
        setLoading(false);
  ...
  const handleSaveBook = () => {
    ...
    axios
      .put(`http://localhost:5555/books/${id}`, {
        ...book,
        publishYear: Number(book.publishYear), // Ensure publishYear is a number for API
      })
      .then(() => {
        setLoading(false);
  ...
  const handleReset = () => {
    ...
    axios
      ...
      .then((response) => {
        const { title, author, publishYear } = response.data;
        setBook({ title, author, publishYear: publishYear.toString() });
        setErrors({});
        setLoading(false);
        enqueueSnackbar("Form reset to original values", { variant: "info" });
      })
      .catch((error) => {
        setLoading(false);
        enqueueSnackbar("Failed to reset form", { variant: "error" });
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="w-full max-w-lg bg-white rounded-2xl shadow-lg p-6 md:p-8">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-gray-800 md:text-3xl">Edit Book</h1>
          <BackButton />
        </div>
